  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit(onSubForm)} className="col-md-6 p-3 shadow">
        ...
        />
        {errors.info && (
          <div className="text-danger">Enter valid info(min 2 chars) </div>
        )}
        <label>Hand:</label>
        <input
          {...register("hand", { required: true, minLength: 1 })}
          type="text"
          className="form-control"
        />
        {errors.hand && (
          <div className="text-danger">Enter valid hand (min 2 chars) </div>
        )}
        <label>Img url:</label>
        <input
        ...
        />
        {errors.img_url && (
          <div className="text-danger">Enter valid url (min 2 chars) </div>
        )}
        <label>Phone:</label>
        <input
        ...
        />
        {errors.price && (
          <div className="text-danger">Enter valid price (min 2 chars) </div>
        )}
        <label>Usernickname:</label>
        <input
          {...register("user_nickname", { required: true, minLength: 2 })}
          type="text"
          className="form-control"
        />
        {errors.user_nickname && (
          <div className="text-danger">
            Enter valid user_nickname (min 2 chars){" "}
          </div>
        )}
        <div className="mt-3">
          <button className="btn btn-success me-5">Add new</button>
          <Link className="btn btn-danger" to="/user/myitems">
            Back
          </Link>
        </div>
      </form>
